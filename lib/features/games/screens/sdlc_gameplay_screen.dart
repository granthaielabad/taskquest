...
class _SdlcGameplayScreenState extends ConsumerState<SdlcGameplayScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(gameEngineProvider, (previous, next) {
      ...
      if (next.status == GameSessionStatus.showingFeedback && previous?.status != GameSessionStatus.showingFeedback) {
        if (next.lastAnswerCorrect == true) {
          ...
          // Reset on wrong
          setState(() => _placedPhases.clear());
        }
      }

      // Clear local state when moving to next question
      if (next.status == GameSessionStatus.playing && previous?.status == GameSessionStatus.showingFeedback) {
        setState(() => _placedPhases.clear());
      }
    });
    ...
    final availablePhases = question.shuffledPhases.where((p) => !_placedPhases.contains(p)).toList();
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Drop Target
                    if (!isAnswered && _placedPhases.length < question.correctOrder.length)
                      DragTarget<String>(
                        onAcceptWithDetails: (d) {
                          setState(() {
                            _placedPhases.add(d.data);
                          });
                          if (_placedPhases.length == question.correctOrder.length) {
                            ref.read(gameEngineProvider.notifier).submitAnswer(_placedPhases);
                          }
                        },
                        builder: (context, candidate, _) => Container(
                          ...
                          child: Center(child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                            Container(width: 32, height: 32, decoration: BoxDecoration(color: colorScheme.onSurface.withValues(alpha: 0.05), borderRadius: BorderRadius.circular(8)), child: Icon(Icons.question_mark_rounded, size: 14, color: colorScheme.onSurfaceVariant.withValues(alpha: 0.2))),
                            const SizedBox(width: 12),
                            Text('Drop next phase here...', style: TextStyle(fontFamily: 'DM Mono', fontSize: 12, color: colorScheme.onSurfaceVariant.withValues(alpha: 0.5))),
                          ])),
                        ),
                      ),
                    ...
                    // Available
                    if (!isAnswered)
                      ...availablePhases.map((p) => Draggable<String>(
                        data: p,
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Text(
                          gameState.lastAnswerCorrect == true ? 'Perfect Order!' : 'Logical Error', 
  ...
  Widget _buildPlacedCard(BuildContext context, int index, String title) {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        const Spacer(),
        Icon(Icons.check_rounded, color: colorScheme.surface.withValues(alpha: 0.5), size: 18),
      ]),
    );
  }

  Widget _buildDraggableCard(BuildContext context, String title, {bool isDragging = false}) {
    final colorScheme = Theme.of(context).colorScheme;
    return Container(
      width: double.infinity,
      margin: const EdgeInsets.only(bottom: 12),
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(18),
        boxShadow: isDragging ? [BoxShadow(color: Colors.black.withValues(alpha: 0.1), blurRadius: 10, offset: const Offset(0, 5))] : null,
      ),
      child: Row(children: [
        Container(width: 36, height: 36, decoration: BoxDecoration(color: colorScheme.onSurface.withValues(alpha: 0.05), borderRadius: BorderRadius.circular(10)), child: Icon(Icons.square_rounded, color: colorScheme.onSurfaceVariant.withValues(alpha: 0.2), size: 14)),
        const SizedBox(width: 16),
        Text(title, style: TextStyle(fontFamily: 'Syne', fontWeight: FontWeight.w800, fontSize: 15, color: colorScheme.onSurface)),
      ]),
    );
  }
}
