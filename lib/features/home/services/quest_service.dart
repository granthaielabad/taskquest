...
class QuestService {
  ...
  Stream<List<QuestModel>> getDailyQuests(String userId, {int? seed}) {
    ...
    // Deterministic selection based on date and user ID
    final effectiveSeed =
        seed ??
        (now.year * 10000 + now.month * 100 + now.day + userId.hashCode);
    ...
    return Rx.combineLatest2<
      ...
      QuerySnapshot,
      List<QuestModel>
    >(questsStream, completedStream, (questsSnap, completedSnap) {
      ...
      // Sort by ID first to ensure shuffle is deterministic across all devices
  ...
  Future<void> completeQuestsByType(
    String userId,
    String category, {
    double accuracy = 0.0,
    String? difficulty,
    ...
    try {
      ...
      for (final quest in dailyQuests) {
        if (quest.category.trim().toUpperCase() == targetCat &&
            !quest.isCompleted) {
          ...
          if (quest.id == 'q11' ||
            ...
            meetsRequirement =
                meetsRequirement && (difficulty?.toUpperCase() == 'HARD');
          }
          ...
          if (quest.id == 'q15' || titleLower.contains('memory master')) {
            ...
            for (var act in activities) {
              if (act.type == ActivityType.study &&
                  act.subtitle.contains('Reviewed')) {
                final match = RegExp(
                  r'Reviewed (\d+) cards',
                ).firstMatch(act.subtitle);
                if (match != null) {
          ...
          if (quest.id == 'q10' || titleLower.contains('polyglot trial')) {
            int playCount = 0;
            for (var act in activities) {
              if (act.type == ActivityType.game &&
                  act.title.contains('Which Lang?')) {
                playCount++;
              }
            }
